...
class Store(Base):
    ...
    inventory: Mapped[List["Inventory"]] = relationship(back_populates='store', cascade='all, delete') # 다른테이블에 있는 관계 변수와 연결
    def __repr__(self):
        return f'<s_id : {self.s_id} name : {self.name}>'

    def to_dict(self): #데이터를 딕셔너리로 구성해서 리턴
...
def get_engine():
    ...
    return engine #플라스크에서 사용할 것을 정의
